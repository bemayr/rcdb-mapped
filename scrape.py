...
def get_max_page_number() -> int:
    return int(max([pagelink.get_text()
                    for pagelink
                    in BeautifulSoup(requests.get(urls["coasters"]["index"]).content, "html.parser").find(
            id="rfoot").find_all("a")
...
def get_coaster_ids(max_page_number) -> [int]:
    return [entry.find_all("a")[1].get("href")[1:].split(".")[0]
            ...
            for entry
            in BeautifulSoup(requests.get(urls["coasters"]["page"] + str(page_number)).content, "html.parser").select(
            "#report tbody")[0].find_all("tr")]
...
def to_base64(coasters: str) -> str:
    return base64.urlsafe_b64encode(str.encode(coasters)).decode('utf-8')
...
def main():
    ...
    coasters = json.dumps(extract_coasters(coaster_ids), cls=CoasterEncoder)
    print("Storing the data in the cloud")
    response = update_gist(to_base64(coasters))
